...
def mds_(dist, km):
    ...
    # convert and plot the cluster centroids
    try:
        centers2D = mds.fit_transform(km.cluster_centers_[:,:5])
        plt.hold(True)
        plt.scatter(centers2D[:,0], centers2D[:,1], marker='x', s=200, linewidths=3)
    except:
        print ("Failed to fit_transform & plot the MDS centroids.")

    return plt

# method 2: principal component analysis (PCA)
def pca_(tfidf_matrix, km):
    global data2D
...
data2D = pd.read_pickle('js/data2D_mds.dat')
linkage_matrix = dendrogram_(data2D) # plot the dendrogram

dendro_to_json(linkage_matrix, 'js/hierarchical_pca', id2name)"""
